...
void motors_update_target_reached(void){
	...
	if(abs(left_motor_pos) >= right_counter_target && position_left_reached == POSITION_NOT_REACHED){
		position_left_reached = POSITION_REACHED;
	}
	if(abs(right_motor_pos) >= left_counter_target && position_right_reached == POSITION_NOT_REACHED ){
